...
function openAddEmployee() {
  ...
  document.getElementById('empUif').value = '';
...
function renderTimesheetView() {
  ...
  view.innerHTML = `
    ...
    <div class="card" style="background:linear-gradient(135deg,rgba(200,169,110,0.1),var(--card));">
      ...
      <div id="tsSummaryRows">${monthlySummaryRows(ts, +emp.rate)}</div>
    </div>
...
function monthlySummaryRows(ts, rate, overrideHrs = null) {
  let totalHrs = overrideHrs !== null ? overrideHrs : 0;
  if (overrideHrs === null && ts) {
    [1,2,3,4].forEach(w => {
...
function refreshSummaryFromDOM() {
  ...
  if (!emp) return;
  let totalHrs = 0;
  [1,2,3,4].forEach(w => DAYS.forEach(d => {
    const el = document.getElementById(`ts_w${w}_${d}`);
    if (el) totalHrs += +el.value || 0;
  }));
  const el = document.getElementById('tsSummaryRows');
  if (el) el.innerHTML = monthlySummaryRows(null, +emp.rate, totalHrs);
}
